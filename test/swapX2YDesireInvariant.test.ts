...
describe("test swapX2YDesireY", ()=>{
    test('test swapX2YDesireY order not cover currentPt or lowPt', () => {
        const liquidityData = [
        ...
        try {
            SwapX2YModule.swapX2YDesireY(pool, JSBI.BigInt('1000000000000000'), -3600)
        } catch (err: any) {
            expect(false).toBe(true)
        }

        try {
            SwapX2YModule.swapX2YDesireY(pool, JSBI.BigInt('1000000000000000'), -3601)
            expect(false).toBe(true)
        } catch (err: any) {
        ...
        try {
            SwapX2YModule.swapX2YDesireY(pool, JSBI.BigInt('1000000000000000'), -3599)
            expect(false).toBe(true)
        } catch (err: any) {
            if (err instanceof SwapQueryError) {
    ...
    test('test swapX2YDesireY if currentPt < lowPt ', () => {
        const liquidityData = [
            getDataPair(-3600, 100000),
            getDataPair(3600, 0)
        ]
        ...
        SwapX2YModule.swapX2YDesireY(pool, JSBI.BigInt('1000000000000000'), currentPoint)
        SwapX2YModule.swapX2YDesireY(pool, JSBI.BigInt('1000000000000000'), currentPoint - 1)
